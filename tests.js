...
function runTests() {
  ...
  describe("Weather App", function () {
    const getLocalTime = (sec) =>
      new Date(sec * 1000)
        .toLocaleTimeString("en-US", { hour: "numeric", minute: "numeric" })
        .toLowerCase();
    const failedWeatherInfo = { cod: "404", message: "city not found" }
    const weatherInfo = {
      "coord": {
        "lon": 139.6917,
        "lat": 35.6895,
    ...
    async function stubWeatherFetch(returnJSON, query, status, ok) {
      const fetchStub = sinon.stub(window, "fetch")
        .resolves({
          json: sinon.stub().resolves(returnJSON),
          status: status || 200,
          ok: ok || true,
        });
    ...
    it('should display "Location Not Found" if no location is found', async function () {
      sinon.restore();
      await stubWeatherFetch(failedWeatherInfo, "askjdnfks", 404, false);
    ...
    it("should display updated time", function () {
      expect(getWeatherHTML().includes(getLocalTime(weatherInfo.dt))).to.be
        .true;
    });
    it("should not display previous location's weather info after searching for a new location", async () => {
      ...
      expect(link).to.exist;
      expect(link.textContent.toLowerCase()).to.eq("click to view map");
      expect(link.href).to.not.eq(
        "https://www.google.com/maps/search/?api=1&query=35.6895,-157.8583",
      );
      const img = document.querySelector("#weather-app img");
      expect(img).to.exist;
      ...
      expect(link.textContent.toLowerCase()).to.eq("click to view map");
      expect(link.href).to.eq(
        "https://www.google.com/maps/search/?api=1&query=21.3069,-157.8583",
      );
